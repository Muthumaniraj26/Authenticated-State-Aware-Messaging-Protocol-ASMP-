...
# Now includes AES-256 encryption and robust key derivation.
...
# --- Configuration ---
SERVER_HOST = '172.16.71.181' # MUST be the IP of your Server PC
SERVER_PORT = 8888
DEVICE_ID = "DEVICE-001"
PRIVATE_KEY_FILE = "device_001_private_key.pem"
DATA_INTERVAL = 5  # seconds
HEARTBEAT_INTERVAL = 40 # seconds
...
class ASMPClient:
    def __init__(self):
        ...
        # [IDENTITY] Load private key for authentication
        self.private_key = self.load_private_key(PRIVATE_KEY_FILE)
        
        # [CONFIDENTIALITY] Use the derived SHA-256 key
    ...
    def load_private_key(self, filename):
        ...
        with open(filename, "rb") as f:
            # In production, this would initialize an HSM key handle
            return serialization.load_pem_private_key(f.read(), password=None)

    def sign_data(self, data):
        """Signs data using the device's private key (Atomic Message Attestation)."""
    ...
    def encrypt_payload(self, data: bytes) -> tuple[bytes, bytes]:
        """Encrypts data using AES-256-CBC and returns ciphertext and IV."""
    ...
    async def connect(self):
        """Connects to the server and performs the Identity-First Handshake."""
